...
export default class EventManager {
     ...
     * 事件订阅表 [事件名, 所有订阅][]
     */
    private static eventSubscribeList: [string, Subscribe[]][] = null;

    /**
     * 订阅者事件表 Map<订阅者,事件名[]>
     */
    private static eventsOfSubscriber: Map<any, string[]> = null;

    static init() {
        if (this.eventSubscribeList === null) {
     ...
     */
    static publishEvent(eventName: string, data: any = null): boolean {
     ...
     * @param false: 已订阅过
     */
    static addSubscribe(subscriber: any, eventName: string, dealFunc: Function): boolean {
        //新订阅
     ...
     */
    static cancelSubscribe(subscriber: any, eventName: string): boolean {
        //找到订阅表
        let subscribes: Subscribe[] = this.findEventSubscribes(eventName);
     ...
     * @param subscriber 订阅者
     */
    static deleteSubscribes(subscriber: any) {
     ...
     */
    private static findEventSubscribes(eventName: string): Subscribe[] {
     ...
     * @returns true: 订阅过
     */
    private static isSubscribed(subscriber: any, eventName: string): boolean {
        let ret = false;
        //得到该订阅者订阅的事件数组
        let events: string[] = this.eventsOfSubscriber.get(subscriber);
     ...
     */
    private static addSubscribEvents(subscriber: any, eventName: string): boolean {
        //获得其订阅的事件表
        let events: string[] = this.eventsOfSubscriber.get(subscriber);
     ...
     */
    private static deleteSubscribEvents(subscriber: any, eventName: string) {
        //得到该订阅者的事件表
        let events: string[] = this.eventsOfSubscriber.get(subscriber);
